...
def save_baidu_info(release_time='', content='', url='', author='', title='', is_debug=False):
    domain = tools.get_domain(url)
    content_info = {
        'domain': domain,
        'title': title,
    ...
    # if not is_debug:
    #     content_info.update({'site_id': site_id, 'read_status': 0, 'record_time': tools.get_current_date()})
